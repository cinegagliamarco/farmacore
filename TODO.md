...
## Deferred from code review (2026-07-30)
- [ ] **CI não roda os testes e2e, e a suíte apodreceu** — `.github/workflows/deploy.yml` roda só `npm run lint` + `npm test`; `npm run test:e2e` nunca roda em CI. Consequência já visível: `test/tenant-api.e2e-spec.ts` tem 7 testes falhando na `main` porque o spec insere o tenant sem `modules` (a coluna default é `'{}'`), então todo endpoint com `@RequireModule` responde 403 desde a PR #66. **Fix do spec:** semear os módulos no `INSERT INTO core.tenant` do `beforeAll`. **Fix do CI:** subir postgres+rabbitmq como services e rodar `test:e2e` (exige migrations aplicadas antes).
- [ ] **FE: tela de Lojas precisa consumir a cota** — `GET /stores/quota` (`{ limit, active }`) para mostrar "X de Y lojas" e desabilitar o toggle quando cheio; tratar `409` (cota cheia, não insistir) e `503` (concorrência, tentar de novo) do `PUT /stores/:id`. Enquanto isso não sai, não definir `storeLimit` em nenhum tenant — o default `NULL` mantém tudo ilimitado.
- [ ] **Admin: campo de quantidade de lojas contratada** — `PUT /admin/tenants/:slug/store-limit` na tela do tenant (aceita `null` para remover o limite).
- [ ] **Contagem de endpoints nas docs está 2 a menos** — `CONTROLLER_MAPPING.md`, `docs/api-reference.md`, `README.md`, `TUTORIAL.md` (3 ocorrências), `postman/README.md` e `postman/WALKTHROUGH.md` dizem 96; o tree tem 98 handlers (offset pré-existente, não introduzido pela cota de lojas). Os 2 que faltam são `GET` e `PATCH /settings/offer-defaults` (`src/tenant-api/config/settings.controller.ts`) — sem seção na referência e sem request no Postman.
- [ ] **`updateStoreLimit`/`updateStatus`/`updateModules` são read-modify-write** — `findBySlug` → mutar → `repo.save(t)`: duas escritas concorrentes de system admin em endpoints diferentes sobrescrevem a coluna uma da outra (lost update). Padrão pré-existente, agora em três endpoints; um `UPDATE ... SET <coluna>` direto resolveria.
