...
**D1 — Relatório próprio × reusar `pricing_apply_run`.** *Rec: próprio.* Contrato do FE é
...
**D2 — Execução assíncrona via worker × síncrona.** *Rec: assíncrona.* Milhares de produtos ×
...
- **Ledger de items congelados**: o `POST /execute` computa a regra e materializa **todos** os
  items na tabela do report na tx inicial — `pending` os que vão atualizar (preço **congelado**),
  `skipped` os pulados. O preço não é recomputado no consumo (redelivery não muda o valor
  escrito).
- **Concorrência por status da regra + recuperação por idade**: `offer_book_rule.status`
  (D5) vira `RUNNING` no `POST`; um `POST` concorrente com `status=RUNNING` **recente** →
  409 (ou 400, D7). Um `RUNNING` **mais velho que `EXECUTE_LOCK_MS`** é reclamável (como
  `ACTIVE_BATCH_LOCK_MS` em `pipeline-run.service.ts:38`) — sem reset no boot.
- **Redelivery segura no consumo**: o consumer **herda `BasePipelineConsumer`** (que roteia a
  duplicata pro DLQ via lock de idade) e empurra **só os items `pending`**, marcando
  `applied`/`failed` por chunk dentro da tx. Redelivery pula os `applied`.
- **Nunca dead-letterar com o report vivo**: o corpo do push **não re-lança** por falha de
  chunk (marca o item `failed` e segue, como `ApplyPriceStep`); o finalizador sempre fecha o
  report/`status` da regra. DLQ só para erro de infra irrecuperável.
...
- **`offer_book_rule` (ALTER)** `[v2]`: `ADD COLUMN status text NOT NULL DEFAULT 'IDLE'` com
  CHECK dos 5 valores.
- **`offer_book_rule_execution_report`**: `rule_id` (FK CASCADE), `offer_book_info_id bigint`,
  ...
  `products_skipped int`, `outcome text CHECK(SUCCESS|FAILURE|NO_CHANGES)`,
...
- **`offer_book_rule_execution_report_item`**: `report_id` (FK CASCADE), `ean bigint`, `name`,
  ...
  `was_updated`), e `[v2]` **`apply_status text CHECK(pending|applied|failed|skipped)`** +
  `apply_error text|null` (o ledger money-safe). Índice em `(report_id, apply_status)`.
- Enums novos `ExecutionType`, `ExecutionOutcome`, `OfferBookRuleStatus` em `src/database/enums/`.
- Campos do item batem 1:1 com o legado (que **não** tem `baseOfferPrice`/`externalId` no item —
  confirmado).

## 5. Execução (sub-fase 3.2) — o núcleo

**Endpoint** `POST /offer-book-rules/:id/execute` (`@Roles(OPERATOR, ADMIN)`), na tx do tenant:
1. 404 se a regra não existe; guard de concorrência por `status` (D3/D5).
...
**Consumer** (worker), herdando `BasePipelineConsumer` `[v2]`, dentro de `runWithTenant`:
- lê os items `pending` do report em chunks; resolve credencial A7 **uma vez**;
- por chunk: `a7.upsertOffer(creds, rule.offerBookInfoId, [{idEmbalagem, precoOferta}...])`,
  depois espelha `offer_book`/`product_item` em bulk, e marca os items `applied`; falha de
  chunk → marca `failed` + `apply_error`, **sem re-lançar**;
...
- **D8** — a wire-message do outbox exige `step: PipelineStep`: ou adicionar
  ...
  `queue: 'execute-offer-book-rule'` (routingKey = `${tenant}.${queue ?? step}`).
...
- `GET /execution-reports` — lista global paginada, filtros `ruleId` (uuid)/
  `offerBookInfoId`/`executionType`/`outcome`/`startDate>=`/`endDate<=` (D7).
...
- **E2e** (`test/offer-book-rules.e2e-spec.ts`, A7 mockado): semear produtos+caderno+regra,
  ...
  Verificar que **redelivery da mesma mensagem não re-empurra** items `applied`.
...
- **Regra presa em `RUNNING`** se o run morre → recuperação por **lock de idade**
  (`EXECUTE_LOCK_MS`), não reset no boot (que atropelaria réplicas).
...
## 11. Checklist
- [ ] Migration: `status` na `offer_book_rule` + realinhar `offer_book_rule_execution_report(_item)`
      com o ledger (`apply_status`/`apply_error`, `idempotency_key`) + enums.
- [ ] `computeForRule` (compute não-paginado reusando o motor).
- [ ] `POST /:id/execute`: compute → materializa items congelados → `rule.status=RUNNING` →
      enfileira via outbox (com `pipelineRunId`+`step`).
- [ ] Consumer herdando `BasePipelineConsumer`; push em lote (`a7.upsertOffer` N/chunk,
      credencial 1×, mirror bulk); marca `applied`/`failed`; nunca re-lança; finaliza
      `outcome`+`rule.status` (D6).
- [ ] Wiring: prover motor no worker; consumer no `CONSUMERS`; `PipelineStep`/queue override;
      channel dedicado prefetch 1.
- [ ] Recuperação por lock de idade (não reset no boot).
- [ ] Os 3 GETs (ordem de rota literal antes de `:id`; nomes de param D7).
- [ ] Unit + e2e (incl. falha parcial e redelivery sem re-push) verdes; lint + build limpos.
