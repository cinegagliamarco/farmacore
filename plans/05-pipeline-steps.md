...
> **Status: ✅ Executed (v2).** Plan 05 v2 is complete. Phase A (dispatcher/batch foundation), Phase B (step ports B1–B5; B6 dropped — see below), Phase C (per-origin scrape: drogal/drogasil/michelassi), Phase D (atomicity + outbox — D1 atomic CTE + D2 outbox; D3 explicitly skipped), and Phase E (DLQ + outbox e2e specs + operator runbook) all landed. Pipeline is green end-to-end with real legacy work in place. `update-base-product-properties` is the terminal step.
>
> **Dropped / skipped:**
> - **B6 (update-active-ingredient-mat) — dropped.** Originally deferred pending product validation; per customer decision the entire MAT feature was removed (commit `9d92b8f`): step + consumer + `PipelineStep.UPDATE_ACTIVE_INGREDIENT_MAT` enum value + `ActiveIngredientEntity` + `ActiveIngredientRepository` + `tenant.active_ingredient` table all deleted. `sync-base-product` stops upserting ingredient names. `shared_catalog.base_product.active_ingredient` text column stays as catalog metadata.
> - **D3 (replace PipelineJoinService with sibling-dispatch check).** Skipped on purpose — moving the join inside a tenant tx introduces a two-phase race. Documented in [`notes/fan-in-atomicity.md`](./notes/fan-in-atomicity.md).
>
> **Original v1 baseline:** Plan 05 was first executed as v1 stubs: 8 step consumers + `pipeline.start` fan-out + 2-branch join all in place and green end-to-end. Bundled fixes: `core.pipeline_run.tenant_id` changed to `text`; `Plan 09 AmqpInterceptor` guarded against `@golevelup` shape.
>
> **Phase A (dispatcher/batch foundation):** pipeline_run schema extensions, BatchPipelineConsumer + DispatchPipelineConsumer base classes, per-origin queue constants — commits `fc8e47f`..`512a8c9`.
>
> **Phase B (step ports):**
...
> - **B4 (calc-base-product-metrics) — ✅ ported.** `tenant_product` gains `margin`, `average_variation`, `status` columns. Step joins `tenant_product` × `shared_catalog.product` (DROGAL+DROGASIL) × `offer_book` per batch of 500 EANs, computes the legacy formula (margin = (basePrice-cost)/basePrice*100; averageVariation against the avg of present competitor prices), and bulk-updates via `UPDATE...FROM(VALUES...)` in one SQL. Status thresholds read from `status_settings.settings` jsonb with the legacy defaults. v1 consumer replaced by dispatch+batch pair; sync-base-product-stock + import-competitor-stock successors now route to `calc-base-product-metrics.dispatch`. Spec covers all status branches.
> - **B5 (update-base-product-properties) — ✅ ported.** `shared_catalog.product` gains `weight`, `height`, `length`, `width`, `supplier`, `brand`. Four passes (supplier → tenant_product.supplier; name → tenant_product.name; weight → base_product.weight; measures → base_product.{height,length,width,weight}) share one dispatch row; the dispatcher queries "missing X" per pass and emits 500-EAN batches across all four under one fan-in counter. DROGASIL preferred over DROGAL with brand as final fallback for supplier (legacy parity). Writes guarded by `WHERE target IS NULL` so cross-tenant writes to shared_catalog don't fight. v1 consumer replaced by dispatch+batch pair; calc-base-product-metrics successor routes to `update-base-product-properties.dispatch`. Spec covers picker fall-through.
> - **B6 (update-active-ingredient-mat) — ❌ dropped.** Originally deferred pending customer validation; resolved as "drop entirely" (commit `9d92b8f`). Removed: `PipelineStep.UPDATE_ACTIVE_INGREDIENT_MAT` enum value, `UpdateActiveIngredientMatStep`, `UpdateActiveIngredientMatConsumer`, `ActiveIngredientEntity`, `ActiveIngredientRepository`, `tenant.active_ingredient` table. `update-base-product-properties` becomes the terminal pipeline step. `sync-base-product` stops upserting ingredient names; `shared_catalog.base_product.active_ingredient` text column survives as catalog metadata.
>
> **Phase C (per-origin scrape) — ✅ ported.** Three scrapers under `src/scrapers/{drogal,drogasil,michelassi}/`: DrogalScraper (VTEX catalog_system + variations + drogalCheckout — sums stock across 2 subsidiaries), DrogasilScraper (search HTML stream-parse for SKU → GraphQL productBySku at drogaraia.com.br → GraphQL stock), MichelassiScraper (instabuy search API; products-only with 429 backoff). PBM detection ported verbatim from legacy. `shared_catalog.product` gains `sku` column + UNIQUE on (ean, origin) for upserts; new `SharedProductRepository.upsertScrapes` and `ProductStockRepository.insertSnapshots`. **import-competitor-products** is a single dispatcher fanning out to 3 per-origin batch queues (drogal=20/batch, drogasil=10/batch, michelassi=1/batch); all batches share one fan-in counter. **import-competitor-stock** same shape with 2 origins (drogal=50/batch, drogasil=30/batch); preserves the v1 PipelineJoinService 'stock-b' branch — last batch fires CALC only when 'stock-a' also done. Per-origin prefetch IS the rate-limit knob: drogal=8/8/4, drogasil=8/8/4, michelassi=2 (matches legacy Promise.all concurrency). All scrapers tested via mocked HttpService; real HTTP validation deferred to operator.
>
> **Phase D (atomicity + outbox) — ✅ done.** Bug #1 (deadlock + lost successor) fully resolved. **D1** atomic CTE in `PipelineRunService.completeBatchAndIncrement` merges complete + counter increment into one SQL; `BatchPipelineConsumer.process` calls it inside the tenant tx. Idempotent on broker redelivery (WHERE status='running'). **D2** new `core.pipeline_outbox` + `OutboxRepository` + `OutboxPublisher` (5s cron, SKIP LOCKED claim). Last-batch successors + empty-dispatch successors stage to outbox inside the same tenant tx as the CTE — the next publisher tick drains them. Dispatcher batches keep direct publish (outboxing 108k import-competitor-products messages would tank latency; `startOrRestartDispatch` covers crash recovery).
>
> **Phase E (validation + docs) — ✅ done.** `test/dlq.e2e-spec.ts` exercises a per-step DLQ end-to-end. `test/outbox.e2e-spec.ts` stages an outbox row, runs `OutboxPublisher.tick()`, and verifies broker delivery + `published_at`. `docs/operator/runbook.md` covers topology, manual triggers, "is the run done?" query, stuck-state diagnosis tree (dispatch stuck, counter stuck, missing branch row with manual recovery SQL), outbox monitoring, DLQ debugging, useful one-liners, and anti-bot incident response.
>
> Background on the v2 design lives in [`notes/pipeline-throughput.md`](./notes/pipeline-throughput.md): per-step dispatcher + per-batch consumer; fan-in counter replaces the v1 two-branch join; per-origin scrape queues with prefetch as the rate-limit knob; ~108k msgs/run/tenant dominated by scrape queues.
