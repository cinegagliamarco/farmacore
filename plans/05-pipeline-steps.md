...
> - **Phase D (atomicity + outbox) — ✅ done.** Bug #1 (deadlock + lost successor) fully resolved. **D1** atomic CTE in `PipelineRunService.completeBatchAndIncrement` merges complete + counter increment into one SQL; `BatchPipelineConsumer.process` calls it inside the tenant tx. Idempotent on broker redelivery (WHERE status='running'). **D2** new `core.pipeline_outbox` + `OutboxRepository` + `OutboxPublisher` (5s cron, SKIP LOCKED claim). Last-batch successors + empty-dispatch successors stage to outbox inside the same tenant tx as the CTE — the next publisher tick drains them. Dispatcher batches keep direct publish (outboxing 108k import-competitor-products messages would tank latency; `startOrRestartDispatch` already covers crash recovery). **D3** (replace `PipelineJoinService` with sibling-dispatch check) — NOT done. Moving the join inside a tenant tx introduces a real two-phase race (both branches' INSERT/COUNT see only their own row before either commits). The current join works because each save is autocommit. Documented in [`notes/fan-in-atomicity.md`](./notes/fan-in-atomicity.md).
> - **Phase C (per-origin scrape) — ✅ ported.** Three scrapers under `src/scrapers/{drogal,drogasil,michelassi}/`: DrogalScraper (VTEX catalog_system + variations + drogalCheckout — sums stock across 2 subsidiaries), DrogasilScraper (search HTML stream-parse for SKU → GraphQL productBySku at drogaraia.com.br → GraphQL stock), MichelassiScraper (instabuy search API; products-only with 429 backoff). PBM detection ported verbatim from legacy.<br/>`shared_catalog.product` gains `sku` column + UNIQUE on (ean, origin) for upserts; new `SharedProductRepository.upsertScrapes` and `ProductStockRepository.insertSnapshots`. **import-competitor-products** is a single dispatcher fanning out to 3 per-origin batch queues (drogal=20/batch, drogasil=10/batch, michelassi=1/batch); all batches share one fan-in counter. **import-competitor-stock** same shape with 2 origins (drogal=50/batch, drogasil=30/batch); preserves the v1 PipelineJoinService 'stock-b' branch — last batch fires CALC only when 'stock-a' also done. Per-origin prefetch IS the rate-limit knob: drogal=8/8/4, drogasil=8/8/4, michelassi=2 (matches legacy Promise.all concurrency). All scrapers tested via mocked HttpService; real HTTP validation deferred to operator (would hit vendor IPs from this session).
>
