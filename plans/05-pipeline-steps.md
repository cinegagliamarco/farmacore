...
> **Status: ✅ Executed (v2).** Plan 05 v2 is complete. Phase A (dispatcher/batch foundation), Phase B (step ports B1–B5; B6 deferred — see below), Phase C (per-origin scrape: drogal/drogasil/michelassi), Phase D (atomicity + outbox — D1 atomic CTE + D2 outbox; D3 explicitly skipped), and Phase E (DLQ + outbox e2e specs + operator runbook) all landed. Pipeline is green end-to-end with real legacy work in place.
>
> **Pending / deferred:**
> - **B6 (update-active-ingredient-mat).** Product decision pending: `active_ingredient` likely moves from `tenant.` to `shared_catalog.` after customer validation; `mat` has no Phase B upstream populator. v1 stub stays wired so the chain completes (B5 → no-op B6 → end). Pulled into the active-ingredient migration when (a) lands.
...
> - **B6 (update-active-ingredient-mat) — ⏸ deferred.** Product decision pending: (a) `active_ingredient` likely moves from `tenant.` to `shared_catalog.` (one catalog for all tenants, validated with a customer first); (b) `mat` has no upstream populator in Phase B and the legacy aggregation source (`base_product.mat`) doesn't exist in the new schema. The v1 stub consumer + step stay wired so the chain completes (B5 → no-op B6 → end). When (a) lands, the active-ingredient migration also pulls B6 (either dropped if MAT is dropped, or pointed at a new per-tenant `mat` source).
>
