...
import { mapProduct, VenancioScraper } from './venancio.scraper';
import type { VenancioProduct } from './types';

const fullProduct: VenancioProduct = {
...
describe('VenancioScraper.scrapeProduct', () => {
  ...
  it('returns found=false when the catalog returns no hits', async () => {
    const out = await new VenancioScraper(buildHttp([])).scrapeProduct('7891');
    expect(out).toEqual({ ean: '7891', origin: 'VENANCIO', found: false });
  });

  it('returns found=false with error on network failure', async () => {
    const http = {
      axiosRef: { get: jest.fn().mockRejectedValue(new Error('ECONNREFUSED')) },
    } as unknown as HttpService;
    const out = await new VenancioScraper(http).scrapeProduct('7891');
    expect(out).toEqual({
      ean: '7891',
      origin: 'VENANCIO',
      found: false,
  ...
  it('maps the first product when the catalog returns a hit', async () => {
    const out = await new VenancioScraper(
