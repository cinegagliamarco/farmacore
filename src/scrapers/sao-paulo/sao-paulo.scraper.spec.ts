...
import { mapProduct, SaoPauloScraper } from './sao-paulo.scraper';
import type { SaoPauloProduct } from './types';

const fullProduct: SaoPauloProduct = {
...
describe('mapProduct (São Paulo)', () => {
  ...
  it('strips HTML and leaves undefined when description is empty', () => {
    const out = mapProduct('7891', { ...fullProduct, description: '<p></p>' });
...
describe('SaoPauloScraper.scrapeProduct', () => {
  ...
  it('returns found=false when the catalog returns no hits', async () => {
    const out = await new SaoPauloScraper(buildHttp([])).scrapeProduct('7891');
    expect(out).toEqual({ ean: '7891', origin: 'SAO_PAULO', found: false });
  });

  it('returns found=false with error on network failure', async () => {
    const http = {
      axiosRef: { get: jest.fn().mockRejectedValue(new Error('ETIMEDOUT')) },
    } as unknown as HttpService;
    const out = await new SaoPauloScraper(http).scrapeProduct('7891');
    expect(out).toEqual({
      ean: '7891',
      origin: 'SAO_PAULO',
      found: false,
  ...
  it('maps the first product when the catalog returns a hit', async () => {
    const out = await new SaoPauloScraper(
