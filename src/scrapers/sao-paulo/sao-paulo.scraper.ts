...
const SEARCH_URL = (ean: string) =>
  `https://www.drogariasaopaulo.com.br/api/catalog_system/pub/products/search?fq=alternateIds_Ean:${ean}`;
...
export class SaoPauloScraper implements ProductScraper {
  public readonly origin = CompetitorOrigin.SAO_PAULO;
  private readonly logger = new Logger(SaoPauloScraper.name);

  constructor(private readonly http: HttpService) {}

  public async scrapeProduct(ean: string): Promise<ScrapedProduct> {
    try {
      const { data } = await this.http.axiosRef.get<SaoPauloProduct[]>(
        SEARCH_URL(ean),
...
export function mapProduct(ean: string, p: SaoPauloProduct): ScrapedProduct {
  ...
  return {
    ...
    brand: p.brand ?? null,
    // VTEX da São Paulo às vezes devolve productReferenceCode null; o SKU
